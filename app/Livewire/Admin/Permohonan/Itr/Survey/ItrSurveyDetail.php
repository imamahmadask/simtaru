...
class ItrSurveyDetail extends Component
{
    ...
    public function download1a()
    {
        ...
        $tgl_survey = $this->itr->tgl_survey ? ucwords((new \NumberFormatter('id', \NumberFormatter::SPELLOUT))->format(\Carbon\Carbon::parse($this->itr->tgl_survey)->day)) : '______';
        $bulan_survey = $this->itr->tgl_survey ? \Carbon\Carbon::parse($this->itr->tgl_survey)->locale('id')->isoFormat('MMMM') : '______';
        $tahun_survey = $this->itr->tgl_survey ? ucwords((new \NumberFormatter('id', \NumberFormatter::SPELLOUT))->format(\Carbon\Carbon::parse($this->itr->tgl_survey)->year)) : '______';
