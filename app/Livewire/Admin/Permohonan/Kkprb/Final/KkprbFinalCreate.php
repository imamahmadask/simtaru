...
class KkprbFinalCreate extends Component
{
    use WithFileUploads;
    ...
    {
        return view('livewire.admin.permohonan.kkprb.final.kkprb-final-create');
    }
    ...
    {
        $no_reg = $this->kkprb->permohonan->registrasi->kode;
        ...
        foreach ($this->persyaratan_berkas as $item) {
            ...
            if (!empty($this->file_[$item->kode])) {
                ...
                $path = $uploadedFile->storeAs(
                    'kkprb/' . $no_reg.'_final_', // folder per registrasi
                    $filename,
                    'public'
                );
