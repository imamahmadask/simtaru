<div>
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="card-body">
                    <table class="table table-borderless">
                        ...
                        <tr>
                            ...
                            <td style="width: 5%">:</td>
                            <td class="text-start">
                                {{ $pelanggaran->tgl_evaluasi ? \Carbon\Carbon::parse($pelanggaran->tgl_evaluasi)->translatedFormat('d F Y') : '-' }}
                            </td>
                        </tr>
                        <tr>
                        ...
                        <div class="d-flex flex-wrap gap-2 mt-2">
                            @if ($pelanggaran->foto_existing)
                                @foreach ($pelanggaran->foto_existing as $foto)
                                    <a href="{{ asset('storage/' . $foto) }}" target="_blank">
                                        <img src="{{ asset('storage/' . $foto) }}" class="img-thumbnail"
                                            style="width: 150px; height: 150px; object-fit: cover;" alt="Foto Evaluasi">
                                    </a>
                                @endforeach
                            @else
                    ...
                    <div class="row g-2 mt-2">
                        @php
                            $templatesSurat = [
                                ...
                                [
                                    'label' => 'Pelimpahan Berkas Pol PP',
                                    'download' => 'downloadSuratPelimpahanBerkasPolPP',
                                    'file' => 'file_pelimpahan_polpp',
                                ],
                                [
                                    'label' => 'Surat Pernyataan',
                                    'download' => 'downloadSuratPernyataan',
                                    'file' => 'file_pernyataan',
                                ],
                                [
                                    'label' => 'Surat Sosialisasi',
                                    'download' => 'downloadSuratSosialisasi',
                                    'file' => 'file_sosialisasi',
                                ],
                            ];
                        @endphp

                        @foreach ($templatesSurat as $tpl)
                            <div class="col-12 col-md-4">
                                <div class="btn-group w-100">
                                    <button wire:click="{{ $tpl['download'] }}"
                                        class="btn btn-outline-primary btn-sm w-100" title="Download Template">
                                        <i class="bx bx-download"></i> {{ $tpl['label'] }}
                                    </button>
                                    @if ($pelanggaran->{$tpl['file']})
                                        <a href="{{ Storage::url($pelanggaran->{$tpl['file']}) }}" target="_blank"
                                            class="btn btn-primary btn-sm" title="Lihat Dokumen Terupload">
            ...
            <div class="card mt-3">
                <div class="card-header">
                    <h5 class="mb-0">Template Berita Acara</h5>
                </div>
                <div class="card-body">
                    <div class="row g-2 mt-2">
                        @php
                            $templatesBa = [
                                [
                                    'label' => 'Upload BA Pengambilan Dokumen',
                                    'download' => 'downloadBAPengambilanDokumen',
                                    'file' => 'file_ba_pengambilan_dokumen',
                                ],
                                [
                                    'label' => 'Upload BA Penolakan',
                                    'download' => 'downloadBAPenolakan',
                                    'file' => 'file_ba_penolakan',
                                ],
                                [
                                    'label' => 'Upload BA Survey Lapangan',
                                    'download' => 'downloadBASurvey',
                                    'file' => 'file_ba_survey',
                                ],
                                [
                                    'label' => 'Upload BA Survey Untuk Surveyor',
                                    'download' => 'downloadBASurveySurveyor',
                                    'file' => 'file_ba_survey_surveyor',
                                ],
                                [
                                    'label' => 'Upload BA Wawancara',
                                    'download' => 'downloadBAWawancara',
                                    'file' => 'file_ba_wawancara',
                                ],
                                [
                                    'label' => 'Upload BA Penerapan Sanksi',
                                    'download' => 'downloadPenerapanSanksi',
                                    'file' => 'file_ba_penerapan_sanksi',
                                ],
                                [
                                    'label' => 'Upload BA Sosialisasi',
                                    'download' => 'downloadBASosialisasi',
                                    'file' => 'file_ba_sosialisasi',
                                ],
                                [
                                    'label' => 'Upload SK  Sanksi Pemberhentian',
                                    'download' => 'downloadSKSanksiPemberhentian',
                                    'file' => 'file_sk_sanksi_pemberhentian',
                                ],
                            ];
                        @endphp

                        @foreach ($templatesBa as $tpl)
                            <div class="col-12 col-md-4">
                                <div class="btn-group w-100">
                                    <button wire:click="{{ $tpl['download'] }}"
                                        class="btn btn-outline-primary btn-sm w-100" title="Download Template">
    ...
    <div wire:ignore.self class="modal fade" id="AnalisaPelanggaranModal" tabindex="-1"
        aria-labelledby="AnalisaPelanggaranModalLabel" aria-hidden="true">
                ...
                <form wire:submit.prevent="storeAnalisa">
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select wire:model.live="temuan_pelanggaran"
                                class="form-select @error('temuan_pelanggaran') is-invalid @enderror"
                                id="temuan_pelanggaran">
                        ...
                        @if ($temuan_pelanggaran == 'Ada Pelanggaran')
                            <div class="mb-3">
                                ...
                                <select wire:model="tindak_lanjut"
                                    class="form-select @error('tindak_lanjut') is-invalid @enderror"
                                    id="tindak_lanjut">
                            ...
                            <div class="mb-3">
                                <label for="foto_tindak_lanjut" class="form-label">Bukti Tindak Lanjut
                                    (Gambar)</label>
                                <input type="file" wire:model="foto_tindak_lanjut"
                                    class="form-control @error('foto_tindak_lanjut.*') is-invalid @enderror"
                                    id="foto_tindak_lanjut" multiple accept="image/*">
                                <div wire:loading wire:target="foto_tindak_lanjut" class="text-info mt-1">Uploading...
                                </div>
                                        ...
                                        <div class="row g-2">
                                            @foreach ($existing_foto_tindak_lanjut as $index => $foto)
                                                <div class="col-3 position-relative">
                                                    <img src="{{ Storage::url($foto) }}" class="img-thumbnail"
                            ...
                            <hr>
                            <h6 class="fw-bold mb-3">Dokumen Template Surat</h6>
                            <div class="row g-2">
                                @php
                                    $uploadFields1 = [
                                        ...
                                        [
                                            'label' => 'Upload Pelimpahan Pol PP',
                                            'id' => 'upload_pelimpahan_polpp',
                                            'current' => 'file_pelimpahan_polpp',
                                        ],
                                        [
                                            'label' => 'Upload Surat Pernyataan',
                                            'id' => 'upload_pernyataan',
                                            'current' => 'file_pernyataan',
                                        ],
                                        [
                                            'label' => 'Upload Surat Sosialisasi',
                                            'id' => 'upload_sosialisasi',
                                            'current' => 'file_sosialisasi',
                                        ],
                                    ];
                                @endphp

                                @foreach ($uploadFields1 as $field)
                                    <div class="col-md-6 mb-3">
                                        <label for="{{ $field['id'] }}"
                                            class="form-label small">{{ $field['label'] }}</label>
                                        <input type="file" wire:model="{{ $field['id'] }}"
                                            class="form-control form-control-sm @error($field['id']) is-invalid @enderror"
                                            id="{{ $field['id'] }}">
                                        <div wire:loading wire:target="{{ $field['id'] }}"
                                            class="text-info x-small">Uploading...</div>
                                        @if ($pelanggaran->{$field['current']})
                                            <div class="mt-1">
                                                <a href="{{ Storage::url($pelanggaran->{$field['current']}) }}"
                                                    target="_blank" class="text-primary x-small">
                            ...
                            <hr>
                            <h6 class="fw-bold mb-3">Dokumen Template Berita Acara</h6>
                            <div class="row g-2">
                                @php
                                    $uploadFields2 = [
                                        [
                                            'label' => 'Upload BA Pengambilan Dokumen',
                                            'id' => 'upload_ba_pengambilan_dokumen',
                                            'current' => 'file_ba_pengambilan_dokumen',
                                        ],
                                        [
                                            'label' => 'Upload BA Penolakan',
                                            'id' => 'upload_ba_penolakan',
                                            'current' => 'file_ba_penolakan',
                                        ],
                                        [
                                            'label' => 'Upload BA Survey Lapangan',
                                            'id' => 'upload_ba_survey',
                                            'current' => 'file_ba_survey',
                                        ],
                                        [
                                            'label' => 'Upload BA Survey Untuk Surveyor',
                                            'id' => 'upload_ba_survey_surveyor',
                                            'current' => 'file_ba_survey_surveyor',
                                        ],
                                        [
                                            'label' => 'Upload BA Wawancara',
                                            'id' => 'upload_ba_wawancara',
                                            'current' => 'file_ba_wawancara',
                                        ],
                                        [
                                            'label' => 'Upload BA Penerapan Sanksi',
                                            'id' => 'upload_ba_penerapan_sanksi',
                                            'current' => 'file_ba_penerapan_sanksi',
                                        ],
                                        [
                                            'label' => 'Upload BA Sosialisasi',
                                            'id' => 'upload_ba_sosialisasi',
                                            'current' => 'file_ba_sosialisasi',
                                        ],
                                        [
                                            'label' => 'Upload SK  Sanksi Pemberhentian',
                                            'id' => 'upload_sk_sanksi_pemberhentian',
                                            'current' => 'file_sk_sanksi_pemberhentian',
                                        ],
                                    ];
                                @endphp

                                @foreach ($uploadFields2 as $field)
                                    <div class="col-md-6 mb-3">
                                        <label for="{{ $field['id'] }}"
                                            class="form-label small">{{ $field['label'] }}</label>
                                        <input type="file" wire:model="{{ $field['id'] }}"
    ...
    <div wire:ignore.self class="modal fade" id="EvaluasiModal" tabindex="-1" aria-labelledby="EvaluasiModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form wire:submit.prevent="storeEvaluasi">
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="date" wire:model="tgl_evaluasi"
                                class="form-control @error('tgl_evaluasi') is-invalid @enderror" id="tgl_evaluasi">
                            @error('tgl_evaluasi')
                        ...
                        <div class="mb-3">
                            ...
                            <select wire:model="status_pelanggaran"
                                class="form-select @error('status_pelanggaran') is-invalid @enderror"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" wire:model="temp_foto_existing"
                                class="form-control @error('temp_foto_existing.*') is-invalid @enderror"
                                id="temp_foto_existing" multiple accept="image/*">
                            <div wire:loading wire:target="temp_foto_existing" class="text-info mt-1 small">
                                Uploading...</div>
                                        ...
                                        @foreach ($existing_foto_existing as $index => $foto)
                                            <div class="col-3 position-relative">
                                                <img src="{{ Storage::url($foto) }}" class="img-thumbnail"
                                                    style="width: 100px; height: 100px; object-fit: cover;">
                                                <button type="button"
                                                    wire:click="removeExistingFotoExisting({{ $index }})"
                                                    class="btn btn-danger btn-xs position-absolute top-0 end-0 m-1"
                                                    title="Hapus">
                                                    <i class="bx bx-x"></i>
                                                </button>
                                        ...
                                        @foreach ($temp_foto_existing as $index => $foto)
                                            <div class="col-3 position-relative">
                                                <img src="{{ $foto->temporaryUrl() }}" class="img-thumbnail"
                                                    style="width: 100px; height: 100px; object-fit: cover;">
                                                <button type="button"
                                                    wire:click="removeTempFotoExisting({{ $index }})"
                                                    class="btn btn-danger btn-xs position-absolute top-0 end-0 m-1"
                                                    title="Hapus">
                                                    <i class="bx bx-x"></i>
                                                </button>
