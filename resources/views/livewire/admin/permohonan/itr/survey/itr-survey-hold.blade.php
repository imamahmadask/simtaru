<div>
    <div wire:ignore.self class="modal fade" id="HoldSurveyItrModal" tabindex="-1" aria-labelledby="HoldSurveyItrModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="HoldSurveyItrModalLabel">Hold Survey ITR</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" wire:model="tgl_survey_hold" class="form-control @error('tgl_survey_hold') is-invalid @enderror" 
                                id="tgl_survey_hold_itr">
                            @error('tgl_survey_hold') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>                        
                        <div class="mb-3">
                            <label for="ket_survey_hold_itr" class="form-label">Keterangan / Alasan Hold</label>
                            <textarea wire:model="ket_survey_hold" class="form-control @error('ket_survey_hold') is-invalid @enderror" 
                                id="ket_survey_hold_itr" rows="3" placeholder="Masukkan alasan hold survey..."></textarea>
...
@script
    <script>
        $wire.on('trigger-close-modal-itr', () => {
            const modal = bootstrap.Modal.getInstance(document.getElementById('HoldSurveyItrModal'));
            if (modal) {
                modal.hide();
            }
        });
