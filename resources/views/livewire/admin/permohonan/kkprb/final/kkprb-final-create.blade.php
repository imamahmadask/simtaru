<div>
    <div wire:ignore.self class="modal fade" id="addDataFinalKkprbModal" data-bs-backdrop="static" tabindex="-1"
        aria-hidden="true">
                                ...
                                <div class="col mb-3">
                                    <div class="d-flex align-items-center">
                                        <label for="file_.{{ $item->id }}" class="form-label">Upload
                                            {{ $item->nama_berkas }} {!! $item->wajib ? '<span class="text-danger">*</span>' : '' !!}
...
@script
    <script>
        $wire.on('trigger-close-modal', () => {
            const modal = bootstrap.Modal.getInstance(document.getElementById('addDataFinalKkprbModal'));
            if (modal) {
                modal.hide();
            }
        });
